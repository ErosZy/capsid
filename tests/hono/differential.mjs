...
import {
    app,
    createReferenceJwt,
} from '../../examples/hono-reference/src/app.js';
...
const replaceCaptures = (value, captures) => value.replace(
    /^\$([a-z][a-z0-9-]*)$/,
    (match, name) => captures.get(name) ?? match,
);

const resolvedHeaders = (vector, captures) => vector.headers.map(
    ([ name, value ]) => [ name, replaceCaptures(value, captures) ],
);
...
const referenceRequest = async (vector, headers) => {
    ...
    const response = await app.request(new Request(vector.url, init));
    const entries = [];
...
const upstream = createServer((request, response) => {
    response.writeHead(203, {
        ...
        'x-hono-upstream': 'direct-txiki-fetch',
    });
...
const upstreamUrl = `http://127.0.0.1:${upstreamPort}/hono-upstream`;
...
const nextLine = async () => {
    ...
    if (result.done) {
        throw new Error('Hono worker driver exited unexpectedly');
    }
...
captures.set('jwt', await createReferenceJwt());
...
for (const vector of selectedVectors) {
    ...
    const headers = resolvedHeaders(activeVector, captures);
    const reference = await referenceRequest(activeVector, headers);
    if (activeVector.captureHeader) {
        const [ headerName, captureName ] = activeVector.captureHeader;
        const value = reference.response.headers.get(headerName);
        assert.ok(value, `${activeVector.id}: reference omitted ${headerName}`);
        captures.set(captureName, value);
    }
...
assert.equal(exitCode, 0, 'Hono worker driver exit status');
...
console.log(
    `PASS: ${selectedVectors.length} Hono differential vectors ` +

...
void exercise(const std::uint8_t* data, size_t size) {
    ...
    // Fixed point: optimizing the output must succeed and change
    // nothing (also re-validates version, checksum and structure).
    std::string error2;
    ...
    // Parse-only pass must accept the optimized bundle too.
    std::string error3;

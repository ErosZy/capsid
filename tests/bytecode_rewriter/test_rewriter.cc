...
// (docs/bytecode-aot-rewriter.md). Part A drives rewrite() on
...
// JS_WriteObject → rewrite → JS_ReadObject → JS_EvalFunction) through
// tjs and compares return values, exceptions, and stack-trace line
// numbers against the raw path.
//
...
struct Builder {
    ...
    bool run_rewrite(std::vector<std::uint8_t>* out, std::string* err,
                      std::uint32_t passes) {
        if (!capsid::bytecode::rewrite(buf, out, passes, false, err)) {
            return false;
        }
        CHECK(out->size() > 5);
    ...
    bool rewrite_and_code(std::vector<std::uint8_t>* out_code,
                           std::string* err, std::uint32_t passes) {
        std::vector<std::uint8_t> out;
        if (!run_rewrite(&out, err, passes)) return false;
        std::size_t p = 5;
    ...
    bool rewrite_and_fcode(std::vector<std::uint8_t>* out_code,
                            std::string* err, std::uint32_t passes) {
        std::vector<std::uint8_t> out;
        if (!run_rewrite(&out, err, passes)) return false;
        std::size_t p = 5;
...
void expect_code(const char* name, Builder* b, const char* hex,
    ...
    std::vector<std::uint8_t> code;
    std::string err;
    if (!b->rewrite_and_code(&code, &err, passes)) {
        std::fprintf(stderr, "FAIL %s: rewrite error: %s\n", name,
                     err.c_str());
        g_failures++;
        return;
    }
...
void expect_fcode(const char* name, Builder* b, const char* hex,
    ...
    std::vector<std::uint8_t> code;
    std::string err;
    if (!b->rewrite_and_fcode(&code, &err, passes)) {
        std::fprintf(stderr, "FAIL %s: rewrite error: %s\n", name,
                     err.c_str());
        g_failures++;
        return;
...
void test_fail_closed_matrix() {
    // version mismatch
    {
        ...
        std::vector<std::uint8_t> out;
        std::string err;
        CHECK(!capsid::bytecode::rewrite(b.buf, &out, 0xffffffffu, false,
                                          &err));
    }
    ...
    // checksum never matches a truncated range)
    {
        ...
        for (std::size_t n = 0; n < b.buf.size(); n++) {
            ...
            std::vector<std::uint8_t> out;
            std::string err;
            CHECK(!capsid::bytecode::rewrite(t, &out, 0xffffffffu, false,
                                              &err));
        }
    }
    ...
    // empty buffer
    {
        std::vector<std::uint8_t> out;
        std::string err;
        CHECK(!capsid::bytecode::rewrite(std::vector<std::uint8_t>(), &out,
                                          0xffffffffu, false, &err));
        CHECK(out.empty());
        CHECK(err == "bytecode rewrite: truncated u8 at offset 0");
    }
}
...
void test_p2_gates() {
    ...
    // object and flows into get_loc.
    {
        ...
        std::vector<std::uint8_t> code;
        std::string err;
        CHECK(b.rewrite_and_code(&code, &err, 0xffffffffu));
...
void test_p11_folds() {
    ...
    // (get,put) pairs vanish -> get_loc a; return.
    {
        ...
        std::vector<std::uint8_t> code;
        std::string err;
        CHECK(b.rewrite_and_code(&code, &err, capsid::bytecode::kPassP11));
...
void test_p11_gates() {
    ...
    // that skips get_loc1/put_loc0 must still read the old slot 0 value.
    {
        ...
        std::vector<std::uint8_t> code;
        std::string err;
        CHECK(b.rewrite_and_code(&code, &err,
...
void test_p14_folds() {
    ...
    // removed). Atom operands are raw u32 values; 0 works on both sides.
    {
        ...
        std::vector<std::uint8_t> code;
        std::string err;
        CHECK(b.rewrite_and_code(&code, &err, capsid::bytecode::kPassP14));
...
void test_debug_block_remap() {
    // Function with a debug block: the rewriter must rewrite the code
    // blob and keep the debug block structurally valid (the rewriter's
...
void test_classic_benchmark_boundary() {
    ...
    if (data != nullptr) {
        ...
        CHECK(!capsid::bytecode::rewrite(raw, &rejected, 0xffffffffu, false,
                                          &product_error));
        CHECK(product_error.find("not a module") != std::string::npos);

        std::vector<std::uint8_t> rewritten;
        std::string error;
        CHECK(capsid::bytecode::rewrite_classic_for_benchmark(
            raw, &rewritten, 0xffffffffu, false, &error));
        JSValue loaded = JS_ReadObject(ctx, rewritten.data(), rewritten.size(),
...
void test_determinism_and_idempotence() {
    ...
    std::string e1, e2, e3;
    CHECK(capsid::bytecode::rewrite(b.buf, &o1, 0xffffffffu, false, &e1));
    CHECK(capsid::bytecode::rewrite(b.buf, &o2, 0xffffffffu, false, &e2));
    CHECK(o1 == o2);
    CHECK(capsid::bytecode::rewrite(o1, &o3, 0xffffffffu, false, &e3));
    CHECK(o1 == o3);
}
...
// Compile + serialize + rewrite + deserialize + eval. When `do_rewrite` is
// false the middle step is skipped (raw control path).
RtResult run_roundtrip(JSContext* ctx, const char* source,
                       const char* name, bool do_rewrite) {
    RtResult r;
    ...
    if (do_rewrite) {
        ...
        std::string err;
        if (!capsid::bytecode::rewrite(buf, &opt, 0xffffffffu, false,
                                        &err)) {
            r.ok = false;
            r.value = "<rewrite failed: " + err + ">";
            return r;
        }
        buf.swap(opt);
    }
...
void test_p11_semantics() {
    ...
    // barriers must keep raw and rewritten execution identical.
    JSRuntime* rt = JS_NewRuntime();
    CHECK(rt != nullptr);
    JSContext* ctx = JS_NewContext(rt);
    CHECK(ctx != nullptr);
...
void test_p14_semantics() {
    ...
    // must keep raw and rewritten execution identical. Covers the
...
int main() {
    ...
    if (g_failures != 0) {
        std::fprintf(stderr, "test_bytecode_rewriter: %d failure(s)\n",
                     g_failures);
        return 1;
    }
    std::fprintf(stderr, "test_bytecode_rewriter: all checks passed\n");
    return 0;
}

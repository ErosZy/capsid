...
void test_minimal_and_full_shapes_are_valid() {
    ...
    require_valid(
        R"json({"apiVersion":"capsid/binding-v1","permissions":{"modules":["tjs:utils"],"net":{"allow":[]}}})json",
        "manifest with an empty net allow list");
}

void test_manifest_envelope_is_exact() {
    require_error(
        R"json({"permissions":{"modules":["tjs:utils"]}})json",
        ConfigErrorCode::kInvalidValue,
        "/apiVersion",
        "manifest without apiVersion");
    require_error(
        R"json({"apiVersion":"capsid/binding-v2","permissions":{"modules":["tjs:utils"]}})json",
        ConfigErrorCode::kInvalidValue,
        "/apiVersion",
        "unsupported manifest apiVersion");
    require_error(
        R"json({"apiVersion":7,"permissions":{"modules":["tjs:utils"]}})json",
        ConfigErrorCode::kInvalidValue,
        "/apiVersion",
    ...
    require_error(
        R"json({"apiVersion":"capsid/binding-v1","permissions":{"modules":["tjs:utils"],"extra":true}})json",
        ConfigErrorCode::kUnknownField,
        "/permissions/extra",
        "unknown manifest permission");
    require_error(
        R"json({"apiVersion":"capsid/binding-v1","sandbox":{"extra":true},"permissions":{"modules":["tjs:utils"]}})json",
        ConfigErrorCode::kUnknownField,
        "/sandbox/extra",
        "unknown manifest sandbox field");
    require_error(
        R"json({"apiVersion":"capsid/binding-v1","mystery":true,"permissions":{"modules":["tjs:utils"]}})json",
        ConfigErrorCode::kUnknownField,
        "/mystery",
...
void test_manifest_modules_are_required_and_known() {
    ...
    require_error(manifest_with_modules(R"json(["tjs:worker"])json"),
                  ConfigErrorCode::kInvalidValue,
                  "/permissions/modules/0",
                  "permanently forbidden worker module");
    require_error(manifest_with_modules(R"json(["tjs:http-server"])json"),
                  ConfigErrorCode::kInvalidValue,
                  "/permissions/modules/0",
                  "permanently forbidden http-server module");
    require_error(manifest_with_modules(R"json(["tjs:process"])json"),
                  ConfigErrorCode::kInvalidValue,
                  "/permissions/modules/0",
    ...
    require_error(manifest_with_modules(R"json(["tjs:utils",7])json"),
                  ConfigErrorCode::kInvalidValue,
                  "/permissions/modules/1",
                  "non-string module entry");
    require_error(manifest_with_modules(R"json(["tjs:utils","tjs:utils"])json"),
                  ConfigErrorCode::kInvalidValue,
                  "/permissions/modules/1",
...
void test_manifest_sandbox_profiles_are_fixed_and_unique() {
    require_valid(
        R"json({"apiVersion":"capsid/binding-v1","sandbox":{"requires":["network-client","filesystem-read","filesystem-write","filesystem-watch","sqlite","wasi"]},"permissions":{"modules":["tjs:utils"]}})json",
        "every defined sandbox profile");
    require_valid(
        R"json({"apiVersion":"capsid/binding-v1","sandbox":{"requires":[]},"permissions":{"modules":["tjs:utils"]}})json",
        "empty sandbox requires list");

    require_error(
        R"json({"apiVersion":"capsid/binding-v1","sandbox":{"requires":["network-server"]},"permissions":{"modules":["tjs:utils"]}})json",
        ConfigErrorCode::kInvalidValue,
        "/sandbox/requires/0",
        "unknown sandbox profile");
    require_error(
        R"json({"apiVersion":"capsid/binding-v1","sandbox":{"requires":["network-client","network-client"]},"permissions":{"modules":["tjs:utils"]}})json",
        ConfigErrorCode::kInvalidValue,
        "/sandbox/requires/1",
        "duplicate sandbox profile");
    require_error(
        R"json({"apiVersion":"capsid/binding-v1","sandbox":{"requires":"network-client"},"permissions":{"modules":["tjs:utils"]}})json",
        ConfigErrorCode::kInvalidValue,
        "/sandbox/requires",
        "non-array sandbox requires");
    require_error(
        R"json({"apiVersion":"capsid/binding-v1","sandbox":7,"permissions":{"modules":["tjs:utils"]}})json",
        ConfigErrorCode::kInvalidValue,
        "/sandbox",
        "non-object sandbox block");
    require_error(
        R"json({"apiVersion":"capsid/binding-v1","sandbox":{"requires":[7]},"permissions":{"modules":["tjs:utils"]}})json",
        ConfigErrorCode::kInvalidValue,
        "/sandbox/requires/0",
...
void test_manifest_permission_shapes_are_typed() {
    require_valid(
        R"json({"apiVersion":"capsid/binding-v1","sandbox":{"requires":["network-client"]},"permissions":{"modules":["tjs:utils"],"net":{"allow":["127.0.0.1:6379"]}}})json",
        "manifest net permission");
    require_valid(
        R"json({"apiVersion":"capsid/binding-v1","sandbox":{"requires":["filesystem-read"]},"permissions":{"modules":["tjs:utils"],"fs":{"read":["/etc/capsid/mongo"],"write":[]},"env":["APP_MODE"],"stdio":["stdout"]}})json",
        "manifest fs/env/stdio permissions");

    require_error(
        R"json({"apiVersion":"capsid/binding-v1","permissions":{"modules":["tjs:utils"],"net":{"allow":["noport"]}}})json",
        ConfigErrorCode::kInvalidValue,
        "/permissions/net/allow/0",
        "invalid manifest net target");
    require_error(
        R"json({"apiVersion":"capsid/binding-v1","permissions":{"modules":["tjs:utils"],"net":{"deny":[]}}})json",
        ConfigErrorCode::kUnknownField,
        "/permissions/net/deny",
        "manifest net deny field");
    require_error(
        R"json({"apiVersion":"capsid/binding-v1","permissions":{"modules":["tjs:utils"],"net":[]}})json",
        ConfigErrorCode::kInvalidValue,
        "/permissions/net",
        "non-object manifest net block");
    require_error(
        R"json({"apiVersion":"capsid/binding-v1","permissions":{"modules":["tjs:utils"],"fs":{"allow":[]}}})json",
        ConfigErrorCode::kUnknownField,
        "/permissions/fs/allow",
        "manifest fs allow field");
    require_error(
        R"json({"apiVersion":"capsid/binding-v1","permissions":{"modules":["tjs:utils"],"fs":{"read":[7]}}})json",
        ConfigErrorCode::kInvalidValue,
        "/permissions/fs/read/0",
        "non-string manifest fs read entry");
    require_error(
        R"json({"apiVersion":"capsid/binding-v1","permissions":{"modules":["tjs:utils"],"env":["1BAD"]}})json",
        ConfigErrorCode::kInvalidValue,
        "/permissions/env/0",
        "invalid manifest environment name");
    require_error(
        R"json({"apiVersion":"capsid/binding-v1","permissions":{"modules":["tjs:utils"],"stdio":[7]}})json",
        ConfigErrorCode::kInvalidValue,
        "/permissions/stdio/0",
...
void test_manifest_profile_permission_consistency() {
    require_error(
        R"json({"apiVersion":"capsid/binding-v1","permissions":{"modules":["tjs:utils"],"net":{"allow":["*:27017"]}}})json",
        ConfigErrorCode::kInvalidValue,
    ...
    require_error(
        R"json({"apiVersion":"capsid/binding-v1","permissions":{"modules":["tjs:utils"],"fs":{"read":["/etc/capsid/mongo"],"write":[]}}})json",
        ConfigErrorCode::kInvalidValue,
        "/permissions/fs/read",
        "fs read permission without the filesystem-read profile");
    require_error(
        R"json({"apiVersion":"capsid/binding-v1","sandbox":{"requires":["filesystem-read"]},"permissions":{"modules":["tjs:utils"],"fs":{"read":[],"write":["/var/lib/capsid/mongo"]}}})json",
        ConfigErrorCode::kInvalidValue,
        "/permissions/fs/write",
        "fs write permission without the filesystem-write profile");
    require_valid(
        R"json({"apiVersion":"capsid/binding-v1","sandbox":{"requires":["filesystem-read","filesystem-write"]},"permissions":{"modules":["tjs:utils"],"fs":{"read":["/etc/capsid/mongo"],"write":["/var/lib/capsid/mongo"]}}})json",
...
void test_manifest_json_envelope_is_strict() {
    require_error(
        R"json({"apiVersion":"capsid/binding-v1","apiVersion":"capsid/binding-v1","permissions":{"modules":["tjs:utils"]}})json",

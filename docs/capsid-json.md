...
- `apiVersion`: must be exactly `capsid/app-v1`; any other value is rejected;
...
In static-pool mode every shard goes through the same load path; if any shard
fails to load, the whole pool fails startup.

## Common errors (all fail closed)

| Invalid input | Result |
| --- | --- |
| `"apiVersion": "capsid/app-v2"` | Rejected: `apiVersion` must be exactly `capsid/app-v1` |

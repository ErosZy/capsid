...
The final artifact retains QuickJS-ng, libuv, WAMR, the Web API implementation, and the DNS/TLS/HTTP client required for standard `fetch()`. The worker's QuickJS heap uses mimalloc by default through txiki's explicit `mi_*` calls; global allocator override remains disabled, so the Host, libuv, and the C++ runtime keep their system allocator. Sanitizer and fuzz builds use the system allocator. The vendor tree is not modified in place: CMake creates an overlay in the build directory and applies `patches/txiki/` in order.

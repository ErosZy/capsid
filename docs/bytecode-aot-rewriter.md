# Bytecode AOT Rewriter

This document is the maintained contract for Capsid's deployed compile-time
bytecode rewriter. Historical implementation plans, intermediate pipelines,
...
rewriter, and only then computes the bundle digest and attestation. The
rewriter:
...
output remains BC26. The rejected custom-opcode and field-IC implementations
have been deleted; the runtime and serializer contain no Capsid bytecode
extension path. Profiling, CFG+SSA, fusion decisions, and the next gate are maintained
...
- `src/bytecode_rewriter/` contains the product library and its internal API;
- `tools/capsid-bytecode-compile.cc` is only a consumer and CLI boundary;
- `tests/bytecode_rewriter/` contains unit and end-to-end differential gates;
- `tests/fuzz/fuzz_bytecode_rewrite.cc` and `tests/fuzz/corpus/bytecode_rewrite/` remain
...
- serialized ordinary opcodes occupy 0..251; values >=252 are invalid in
  BC26. Capsid defines no additional serialized or runtime-only opcode;
...
non-convergence makes `rewrite()` return false with `out` untouched.
...
- compile-only → serialize → rewrite → deserialize → execute round trips,
  including exceptions and stack-trace locations;
- the frozen `runtime_bytecode_compiler_round_trip` RED test, including
  deterministic output;
- source-worker versus rewritten-bytecode-worker differential tests over all
  fixtures;
- ASan+UBSan `fuzz-bytecode-rewrite`, seeded with deterministically compiled
  bundles; output must parse, verify and remain stable under a second rewrite;
- build identity, attestation and managed trusted-bytecode integration tests.

Performance attribution always compares rewritten bytecode with raw
...
P16's synthetic `arith-rt` loop moved from 76 to 26 rewritten instructions;
the three recorded runs measured rewritten-vs-raw at +83.8%..+84.7%. This is a

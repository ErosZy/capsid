...
The project is currently at `0.2.0-beta` (C ABI v7); security fixes target the latest code line, and there is no commitment yet to long-term security maintenance for older versions. The first-party `capsid-host` is currently used for development, integration, and benchmarks; it is not a stable production deployment interface. The Linux strict sandbox is the v1 production isolation target; macOS native-dev, Windows native-dev, and any non-isolated mode must not be used to run untrusted code. For platform differences, see [platform support overview](docs/platform-support.md).

// Bytecode AOT rewriter implementation (design: docs/bytecode-aot-rewriter.md).
//
...
// conditions; see PassFlags in bytecode_rewriter.h.
// Fail-closed: any input the rewriter cannot fully parse, or any
...
public:
    ...
    // The rewriter never interprets atoms; it skips them.
    bool skip_atom() {
        uint32_t v;
        return leb128(&v);
    }
...
private:
    ...
    bool fail(const char* msg) {
        if (error_ && error_->empty()) {
            *error_ = std::string("bytecode rewrite: ") + msg +
...
bool parse_buffer(const uint8_t* data,
    ...
    if (data == nullptr || size == 0) {
        *error = "bytecode rewrite: truncated u8 at offset 0";
        return false;
    }
    ...
    if (version != BC_VERSION) {
        *error = "bytecode rewrite: unsupported bytecode version " +
    ...
    if (stored_csum != actual_csum) {
        *error = "bytecode rewrite: checksum mismatch";
        return false;
    }
    uint32_t atom_count = 0;
    if (!r.leb128(&atom_count)) return false;
    if (atom_count > 1000000) {
        *error = "bytecode rewrite: atom count too large";
        return false;
    }
    ...
    if (top_level == TopLevelKind::kModule) {
        if (tag != BC_TAG_MODULE) {
            *error = "bytecode rewrite: top-level record is not a module";
            return false;
        }
        ...
        if (tag != BC_TAG_FUNCTION_BYTECODE) {
            *error =
                "bytecode rewrite: top-level record is not a global function";
            return false;
        }
        if (!r.skip_function(functions, error, 0, top_level_off)) return false;
    }
    if (!r.at_end()) {
        *error = "bytecode rewrite: trailing bytes after top-level record";
        return false;
    }
    return true;
}
...
// docs/bytecode-aot-rewriter.md); the estimate is deliberately
// conservative.
...
bool decode_code(const uint8_t* code,
    ...
    while (pc < len) {
        uint8_t op = code[pc];
        if (op == 0 || op >= OP_COUNT) {
            *error = "bytecode rewrite: invalid opcode in function";
            return false;
        }
        const OpInfo& oi = short_opcode_info(op);
        uint8_t size = oi.size;
        if (pc + size > len) {
            *error = "bytecode rewrite: truncated instruction";
            return false;
        }
        Insn in;
        in.op = op;
    ...
    for (size_t i = 0; i < insns->size(); i++) {
        ...
        if (want < 0 || static_cast<size_t>(want) >= len) {
            *error = "bytecode rewrite: jump target out of range";
            return false;
        }
        ...
        if (it == offs.end() || *it != static_cast<uint32_t>(want)) {
            *error = "bytecode rewrite: jump target not on instruction "
                     "boundary";
            return false;
        }
...
bool emit_code(const std::vector<Insn>& insns,
    ...
    if (size_changed) {
        *error = "bytecode rewrite: jump sizing did not converge";
        return false;
    }
    // Final offsets.
    new_offs->resize(work.size());
    ...
    for (size_t i = 0; i < work.size(); i++) {
        ...
        default: {
            ...
            if (short_opcode_info(in.op).size != in.old_size) {
                *error = "bytecode rewrite: internal error, operand width "
...
bool verify_code(const uint8_t* code,
                 ...
                 std::string* error) {
    std::vector<Insn> insns;
    if (!decode_code(code, len, &insns, error)) {
        return false;
    }
    if (insns.empty()) {
        *error = "bytecode rewrite: empty code blob";
        return false;
    }
    ...
    auto seed = [&](size_t idx, int32_t h, int32_t catch_pos) -> bool {
        if (idx >= insns.size()) {
            *error = "bytecode rewrite: control flow falls off the code "
                     "blob";
            return false;
        }
        if (heights[idx] != -1) {
            if (heights[idx] != h) {
                *error = "bytecode rewrite: inconsistent stack height at "
            ...
            if (catch_positions[idx] != catch_pos) {
                *error = "bytecode rewrite: inconsistent catch position at "
    ...
    while (!worklist.empty()) {
        ...
        int32_t n_push = oi.n_push;
        if (h < n_pop) {
            *error = "bytecode rewrite: stack underflow at instruction " +
                     std::to_string(idx);
            return false;
        }
        ...
        case OP_nip_catch:
            if (catch_pos < 0) {
                *error = "bytecode rewrite: nip_catch without active catch "
    ...
    if (static_cast<uint32_t>(max_h) > recorded_stack_size) {
        *error = "bytecode rewrite: max stack height exceeds the recorded "
                 "stack size";
        return false;
    }
    return true;
    ...
    *changed = false;
    ...
    if (f.dbg_pc2line_len) {
        if (!decode_pc2line(data + f.dbg_pc2line_off, f.dbg_pc2line_len,
            ...
            *error = "bytecode rewrite: malformed pc2line table";
            return false;
        }
        std::vector<uint32_t> offs;
        offs.reserve(insns.size());
        ...
        for (size_t i = 0; i < pc2line_entries.size(); i++) {
            ...
            if (it == offs.end() || *it != want) {
                *error = "bytecode rewrite: pc2line entry not on an "
    ...
    for (int round = 0; round < 16; round++) {
        ...
        if (round == 15) {
            *error = "bytecode rewrite: rewrite did not converge";
            return false;
        }
    }
    ...
    // not run (defensive; it always runs in rewrite/analyze_only).
...
// rewriter is about to emit (post-fixpoint), so its verdicts reflect
...
// rewriter's internal namespace while the tier-3 section below sits at
...
// rewriter vendor-independent. Only the four emitted by ordinary code
...
static bool optimize_with_top_level(const std::vector<std::uint8_t>& in,
    ...
    if (functions.size() != 1) {
        *error = "bytecode rewrite: top level does not contain exactly one "
                 "function record";
        return false;
    }
    ...
    if (!any_changed) {
        ...
        if (report) {
            std::fprintf(stderr,
                         "bytecode rewrite: %llu -> %llu insns, %llu -> "
    ...
    if (check.size() != 1 ||
        ...
        if (error->empty()) {
            *error = "bytecode rewrite: internal verification failed";
        }
        return false;
    }
    if (report) {
        std::fprintf(stderr,
                     "bytecode rewrite: %llu -> %llu insns, %llu -> %llu "
...
bool rewrite(const std::vector<std::uint8_t>& in,
              std::vector<std::uint8_t>* out,
              uint32_t passes,
              bool report,
              std::string* error) {
...
bool rewrite_classic_for_benchmark(const std::vector<std::uint8_t>& in,
                                    std::vector<std::uint8_t>* out,
                                    uint32_t passes,
                                    bool report,
                                    std::string* error) {

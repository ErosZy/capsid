// Bytecode AOT rewriter for the trusted-bytecode path (design:
// docs/bytecode-aot-rewriter.md). Post-serialization, capsid-side:
...
// the output with JS_ReadObject as-is. The deployed rewriter emits
// canonical BC26 only.
//
// Fail-closed contract: any input the rewriter cannot fully parse
...
#ifndef CAPSID_SRC_BYTECODE_REWRITER_BYTECODE_REWRITER_H
#define CAPSID_SRC_BYTECODE_REWRITER_BYTECODE_REWRITER_H
...
// fixture). Full adjudication is in docs/bytecode-aot-rewriter.md.
...
// docs/bytecode-aot-rewriter.md.
// The format passes (P6 re-shorten, compaction, verification) always
// run when the rewriter runs and are not separately measurable.
enum PassFlags : uint32_t {
    ...
    // Tier-2 direct passes (docs/bytecode-aot-rewriter.md):
    ...
    // Tier-3 deployed passes (docs/bytecode-aot-rewriter.md §3):
...
// Rewrites `in` (a serialized quickjs-ng bytecode buffer) into `out`.
...
// remain module-only and call rewrite(); this function exists solely so the
...
// Returns false on any parse failure, same contract as rewrite().
...
// divergence or parse failure, same contract as rewrite().
...
#endif  // CAPSID_SRC_BYTECODE_REWRITER_BYTECODE_REWRITER_H
